...
```
With the new package installed we can now use it to do this conversion. The 'readr' package has a handy function for converting columns into numbers without us having to write a regular expression and doing a string substitution to remove all the non-number charcters.  So using 'readr' we can simply write:
```{r}
...
```{r}
sorted_spending_df <- temp_spending_df2[order(-temp_spending_df2$spending),] #descedning sort by spending
head(sorted_spending_df, 10) # return the first 10 rows
```
We can also sort by multiple columns - so lets sort by "specialty" and descending "spending":

```{r}
sorted_spending_df <- temp_spending_df2[order(temp_spending_df2$specialty,-temp_spending_df2$spending),]
head(sorted_spending_df, 10) # return the first 10 rows
```
